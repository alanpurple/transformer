...
from tensorflow_core.python.keras import Model,layers
...
class Transformer(Model):
    ...
    def encode(self,inputs,attention_bias,training):
        ...
        with tf.name_scope('encode'):
            ...
            if training:
                encoder_inputs=layers.Dropout(self.params['layer_postprocess_dropout'])(encoder_inputs)

            return self.encoder_stack(encoder_inputs,attention_bias,inputs_padding,training=training)

    def decode(self,targets,encoder_outputs,attention_bias,training):
        """Generates logits for each value in the target sequence.
    ...
    def _get_symbols_to_logits_fn(self,max_decode_length,training):
        ...
        decoder_self_attention_bias=model_utils.get_decoder_self_attention_bias(
            max_decode_length,self.params['dtype']
        )
...
class PrePostProcessingWrapper(layers.Layer):
    """Wrapper class that applies layer pre-processing and post-precessing."""
...
class EncoderStack(layers.Layer):
    ...
    def build(self,input_shape):
        ...
        for _ in range(params['num_hidden_layers']):
            ...
            feed_forward_network=ffn_layer.FeedForwardNetword(
    ...
    def call(self,encoder_inputs,attention_bias,input_padding,training):
...
class DecoderStack(layers.Layer):
    ...
    def build(self,input_shape):
        ...
        self.output_normalization=layers.LayerNormalization(epsilon=1e-6,dtype=tf.float32)
        super().build(input_shape)
